<template>
  <v-app>
    ...
    >
      <v-hover>
        <router-link :to="{ name: 'Projects' }" slot-scope="{ hover }">
          <v-icon
            :size="hover ? 40 : 30"
            color="primary"
          >
      ...
      >
        <v-hover>
          <div slot-scope="{ hover }">
            <span :class="`${hover ? 'primary' : 'secondary'}--text mr-5 subheading`">
              {{ item.title }}
            </span>
          </div>
        </v-hover>
    ...
    <app-mobile v-if="$vuetify.breakpoint.xs"
      :menu="menu"
    >
      Current Page
    </app-mobile>
...
export default {
  ...
  data () {
    return {
      menu: [
        { title: 'Projects', router: 'Projects' },
